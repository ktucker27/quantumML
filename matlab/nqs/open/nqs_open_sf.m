function [S,F] = nqs_open_sf(a, b, c, w, u, lloc, density, num_samps, num_steps)
...
for j=1:num_samps
    [szl, szr] = nqs_open_sample(@nqs_density, a, b, c, w, u, num_steps);
    xib_l = tanh(b + w*szl);
...
sidx = sidx + n;
omat(sidx:sidx+n-1,:) = 1i*(szl_samps - szr_samps); % Im
sidx = sidx + n;
% b
omat(sidx:sidx+m-1,:) = xib_l_mat + xib_r_mat;      % Re
sidx = sidx + m;
omat(sidx:sidx+m-1,:) = 1i*(xib_l_mat - xib_r_mat); % Im
sidx = sidx + m;
% c
